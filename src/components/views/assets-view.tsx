...
export function AssetsView({
  ...
  const selectedAsset =
    rows.find((row) => row.id === selectedAssetId) ?? rows[0]
  const filterOptions =
    section === 'images'
      ? imageCategories
      : section === 'audio'
      ...
      <section className="flex h-full flex-col overflow-hidden">
        ...
        >
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              selectedAsset?.path && onCopy(selectedAsset.path, '资源路径')
            }
          ...
          <Button
            ...
            onClick={() =>
              selectedAsset &&
              onCopy(assetReference(selectedAsset), 'RenPy 引用')
            }
        ...
        <div className="space-y-3">
          ...
          {selectedAsset.character && (
            <CharacterEditor
              ...
              onUpdate={(patch) =>
                onUpdateCharacter(selectedAsset.character!.id, patch)
              }
            />
          )}
          ...
          {selectedAsset.chapter && (
            <ChapterEditor
              ...
              onUpdate={(patch) =>
                onUpdateChapter(selectedAsset.chapter!.id, patch)
              }
              onJumpToLine={onJumpToLine}
            />
          )}
    ...
    <div className="space-y-3 rounded-md border border-border bg-secondary p-3">
      ...
      <label className="grid gap-1">
        ...
        <input
          ...
          value={chapter.order}
          onChange={(event) =>
            onUpdate({ order: Number(event.target.value) || 0 })
          }
          className="h-8 rounded-md border border-border bg-card px-2 text-sm"
        />
      </label>
        ...
        >
          <div className="space-y-2">
            {rules
              ...
              .map((rule) => {
                const stats = rulePreviewStats.get(rule.id) ?? {
                  matched: 0,
                  applied: 0,
                }
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      activeRuleId === rule.id &&
                        'border-info ring-1 ring-info/30',
                    )}
                  >
                    <input
                      type="text"
                    ...
                    <div
                      ...
                      title="命中为 glob 匹配数；生效为按优先级决定分类的资源数"
                    >
                      <Badge
                        variant={
                          rule.enabled && stats.applied > 0 ? 'info' : 'muted'
                        }
